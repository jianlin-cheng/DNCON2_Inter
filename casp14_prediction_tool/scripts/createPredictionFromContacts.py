...
def removeIntraContactsVicinity(l,rr,rr_intra,v=0):#v=relaxation value for removal
    rr_copy=copy.deepcopy(rr)#rr.copy()
...
contact_list=[]
usage_inter="python "+sys.argv[0]+".py <rr_file_predicted> <rr_file_intra_true> <parent_dir_name> <filter_confidence=True/False> <top=5,10,L/10,L/5,L/2,L,2L or all >" # 6BZE_AE.rr 6BZE_A.dncon2.rr 6BZE_AA.rr 

#top_dict={}

if (len(sys.argv)!=6): 
    print(len(sys.argv))
    sys.exit("Please enter 5 valid arguments\n"+usage_inter)
else:
    rr_file2=sys.argv[1] #the prediction file or .dncon2.rr file
    rr_file_intra=sys.argv[2] #the true intra chain contacts
    dirnm=sys.argv[3]
    filter_conf=sys.argv[4]
    trim=sys.argv[5] #top
    print ("Running: "+sys.argv[0]+ " for "+rr_file2)
    #print ("Here I am #######")
    #print (rr_file1)
    #sort the dncon2.rr file
    os.system("python "+dirnm+"/casp14_prediction_tool/scripts/sortrr.py "+rr_file2+" True > "+rr_file2.replace(".rr","_sorted.rr"))
    ...
    if (filter_conf.upper()=="TRUE" or filter_conf.upper()=="T"):
        filter_conf=True
    
    elif (filter_conf.upper()=="FALSE" or filter_conf.upper()=="F"):
        filter_conf=False
    else:
        sys.exit("Please enter True or false for filter_confidence\n"+usage_inter)
    L=len(fasta3)
    if (trim=="5"): 
        top_val=5
    elif (trim=="10"): 
        ...
        sys.exit("Please enter a valid top value\n"+usage_inter)
#    print (L,fasta2)
#    sys.exit()    
    ...
    if (filter_conf==True):rr2=filterConfidence(rr2) #remove contacts < 0.5 probability
    ...
    print ("Selecting top ",top_val,"contacts...")
    write2File(makeOutputFile(rr_file2,"_filtered.rr"), fasta2, trimRR(rr2,top_val))
    write2File(makeOutputFile(rr_file_intra,"_filtered.rr"), fasta3, rr_intra)
    #rr1=trimRR(rr1,2*L)
    ########### Dont need to trim ###########
    #rr2=trimRR(rr2,top_val) #only take top top_val
    #print (rr2)
    #print (makeOutputFile(rr_file2,"_filtered.rr"))
    #sys.exit()
    #print (rr2)
    #findInRR("2 125",rr2)
    #rr_intra=trimRR(rr_intra,2*L)
    
    relaxation=3 #relaxation_removal = 0, 1, 2
    
    orig_rr2=copy.deepcopy(rr2)#rr2.copy()
    #print(rr2)
